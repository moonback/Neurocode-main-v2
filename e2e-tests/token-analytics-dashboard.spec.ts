...
test.describe("Token Analytics Dashboard", () => {
  ...
  test("should display the analytics dashboard", async ({ page }) => {
    ...
    await page.waitForSelector("text=📊 Analytics des Tokens", {
      timeout: 10000,
    });
  ...
  test("should display charts", async ({ page }) => {
    ...
    await page.waitForSelector("text=🔥 Top 5 Conversations", {
      timeout: 10000,
    });
  ...
  test("should display detailed tables", async ({ page }) => {
    ...
    await page.waitForSelector("text=📋 Détail des Conversations", {
      timeout: 10000,
    });

    // Check that table titles are visible
    await expect(
      page.locator("text=📋 Détail des Conversations"),
    ).toBeVisible();
    await expect(
      page.locator("text=💵 Détail des Coûts par Modèle"),
    ).toBeVisible();
  });
  ...
  test("should be accessible from settings page", async ({ page }) => {
    ...
    const analyticsLink = page.locator(
      'a:has-text("📊 View Token Analytics Dashboard")',
    );
