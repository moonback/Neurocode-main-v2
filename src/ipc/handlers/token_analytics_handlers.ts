...
export function registerTokenAnalyticsHandlers() {
  ...
  handle(
    tokenAnalyticsContracts.getStatistics.channel,
    async (_, params) => {
  ...
  handle(
    tokenAnalyticsContracts.getTopConsumers.channel,
    async (_, params) => {
      const manager = getTokenManager();
  ...
  handle(
    tokenAnalyticsContracts.calculateCost.channel,
    async (_, params) => {
      const manager = getTokenManager();
  ...
  // Export usage data
  handle(
    tokenAnalyticsContracts.exportUsageData.channel,
    async (_, params) => {
      const manager = getTokenManager();
  ...
  handle(
    ...
    async (_, params) => {
      ...
      if (stats.totalRequests > 0) {
        ...
        const startTime = params.startDate || Date.now() - 30 * 24 * 60 * 60 * 1000;
        dataPoints.push({
          timestamp: startTime,
