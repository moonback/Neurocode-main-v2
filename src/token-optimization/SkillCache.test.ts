...
describe("SkillCache", () => {
  ...
  describe("constructor", () => {
    ...
    it("should throw error for non-positive idleTimeout", () => {
      expect(() => new SkillCache(10, 0)).toThrow(
        "idleTimeoutMinutes must be positive",
      );
      expect(() => new SkillCache(10, -1)).toThrow(
        "idleTimeoutMinutes must be positive",
      );
    });
  });
  ...
  describe("put and get", () => {
    ...
    it("should throw error for non-positive size", () => {
      expect(() => cache.put("key1", "value1", 0)).toThrow(
        "size must be positive",
      );
      expect(() => cache.put("key1", "value1", -1)).toThrow(
        "size must be positive",
      );
    });
