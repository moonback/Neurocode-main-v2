...
describe('SkillEngine', () => {
  ...
  describe('executeSkill', () => {
    it('should execute a skill successfully', async () => {
      const result = await engine.executeSkill('test-skill', { param: 'value' });
    ...
    it('should handle execution errors', async () => {
      ...
      const result = await engine.executeSkill('nonexistent', { param: 'value' });
    ...
    it('should respect cache disabled config', async () => {
      ...
      // First execution
      const result1 = await noCacheEngine.executeSkill('test-skill', inputs, true);
      expect(result1.fromCache).toBe(false);

      // Second execution should not be from cache
      const result2 = await noCacheEngine.executeSkill('test-skill', inputs, true);
      expect(result2.fromCache).toBe(false);
    });
  ...
  describe('unloadSkill', () => {
    ...
    it('should support unload-reload idempotence', async () => {
      // Load skill
      const result1 = await engine.executeSkill('test-skill', { param: 'value' });

      // Unload
      engine.unloadSkill('test-skill');

      // Reload
      const result2 = await engine.executeSkill('test-skill', { param: 'value' });
  ...
  describe('edge cases', () => {
    ...
    it('should handle special characters in skill names', async () => {
      const result = await engine.executeSkill('skill:with:colons', { param: 'value' });
      expect(result.success).toBe(true);
    });
