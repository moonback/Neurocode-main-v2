...
describe("safeJoin", () => {
  ...
  describe("safe paths", () => {
    it("should join simple relative paths", () => {
      ...
      expect(result).toBe(
        path
          .join(testBaseDir, "src", "components", "Button.tsx")
          .replace(/\\/g, "/"),
      );
    });
    ...
    it("should handle single file names", () => {
      ...
      expect(result).toBe(
        path.join(testBaseDir, "package.json").replace(/\\/g, "/"),
      );
    });
    ...
    it("should handle nested directories", () => {
      ...
      expect(result).toBe(
        path.join(testBaseDir, "src/pages/home/index.tsx").replace(/\\/g, "/"),
      );
    });
    ...
    it("should handle paths with dots in filename", () => {
      ...
      expect(result).toBe(
        path.join(testBaseDir, "config.test.js").replace(/\\/g, "/"),
      );
    });
    ...
    it("should handle empty path segments", () => {
      ...
      expect(result).toBe(
        path.join(testBaseDir, "", "src", "", "file.ts").replace(/\\/g, "/"),
      );
    });
    ...
    it("should handle multiple path segments", () => {
      ...
      expect(result).toBe(
        path
          .join(testBaseDir, "a", "b", "c", "d", "file.txt")
          .replace(/\\/g, "/"),
      );
    });
    ...
    it("should work with actual temp directory", () => {
      ...
      expect(result).toBe(
        path.join(tempDir, "test", "file.txt").replace(/\\/g, "/"),
      );
    });
  ...
  describe("edge cases", () => {
    it("should handle Windows-style base paths", () => {
      ...
      expect(result).toBe(
        path.join(testBaseDirWindows, "src", "file.txt").replace(/\\/g, "/"),
      );
    });
    ...
    it("should handle current directory references safely", () => {
      const result = safeJoin(testBaseDir, "./src/file.txt");
      expect(result).toBe(
        path.join(testBaseDir, "./src/file.txt").replace(/\\/g, "/"),
      );
    });
  ...
  describe("boundary conditions", () => {
    ...
    it("should handle paths that approach but don't cross boundary", () => {
      ...
      expect(result).toBe(
        path.join(testBaseDir, "deep/nested/../file.txt").replace(/\\/g, "/"),
      );
    });
